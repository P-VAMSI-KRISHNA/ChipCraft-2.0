...
const TeamSearch = () => {
  ...
  const { problemsReleased } = useProblemRelease();
    ...
    <section id="search" className="py-12 sm:py-20">
      <div className="container px-4 sm:px-6">
        <div className="mx-auto max-w-2xl">
          ...
          {/* Not Released Gate */}
          {!problemsReleased && (
            <Card className="vintage-border bg-card/60 vintage-shadow text-center py-12">
              <CardContent className="flex flex-col items-center gap-4">
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-amber-500/10 border-2 border-amber-500/30">
                  <Lock className="h-8 w-8 text-amber-500" />
                </div>
                <h3 className="font-rye text-xl sm:text-2xl text-amber-600 uppercase tracking-widest">
                  Not Yet Released
                </h3>
                <p className="text-muted-foreground font-serif text-base sm:text-lg max-w-md">
                  Problem statements have not been released yet. Please wait for the official announcement from the organizers.
                </p>
              </CardContent>
            </Card>
          )}

          {/* Search & Results — only visible when released */}
          {problemsReleased && (
            <>
